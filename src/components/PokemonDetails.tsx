...
const PokemonDetails = ({ pokemon }: PokemonDetailsProps) => {
    return (
        <div className="min-w-[300px] flex flex-col justify-between items-center min-h-[200px] mt-4 p-3 border-2 rounded bg-[#f8f9fa] shadow-xl">
            <h2 className="text-2xl text-center text-[#f4a261]">{pokemon.name}</h2>
            <img src={pokemon.sprites.front_default} alt={pokemon.name} width={160}/>
            <p className="text-[#ccc5b9]">Height: {pokemon.height}</p>
            <p className="text-[#ccc5b9]">Weight: {pokemon.weight}</p>
        </div>
    );
};
